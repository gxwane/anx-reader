...
void main() {
  ...
  group('Archived Book Notes & DAO selectBooksByIds Query Spec', () {
    ...
    test('deleteAllNotesByBookId hard deletes one book notes only', () async {
      ...
      expect(affectedRows, equals(3));

      final notes100 = await db.query(
        'tb_notes',
        where: 'book_id = ?',
        whereArgs: [100],
      );
      expect(notes100, isEmpty);

      final notes200 = await db.query(
        'tb_notes',
        where: 'book_id = ?',
        whereArgs: [200],
      );
      expect(notes200.length, equals(1));
      expect(notes200.first['content'], equals('Other Book Note'));
    });
  });
}
