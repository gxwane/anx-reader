...
import { convertChineseDocument, getOpenCCConverter } from './chinese-converter.js'
...
const convertChineseHandler = (mode, doc) => {
  if (mode && mode !== 'none') {
    convertChineseDocument(mode, doc)
  }
}
...
class Reader {
  ...
  async getTOC() {
    ...
    const converter = getOpenCCConverter(readingRules.convertChineseMode)
    const buildItems = async (items, level) => Promise.all((items ?? []).map(async item => {
      ...
      const label = converter && item.label ? converter(item.label) : item.label
      return {
        label,
        href: item.href,
        id: item.id,
