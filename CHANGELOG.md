...
### 变更
- **WebDAV 同步能力整体回退（WebDAV Sync Rollback）**：自研 WebDAV 同步（v1 边车微同步与 v2 不可变对象引擎）因非收敛缺陷整体回退，应用恢复为上游原版整库快照同步行为；preview.6/7 期间的云端微同步、分组 UUID 映射、Markdown 笔记镜像与冲突副本保护能力一并移除。旧云端同步元数据（`sync/`、`latest_progress.json`、`markdown_notes/` 等）需人工移出同步命名空间归档，已发布版本中已回退的同步功能条目已从本日志撤下。
